...
# ── Minimal env so config initialises without a .env file ────────────────────
...
def test_sign_and_verify_roundtrip(keystore: KeyStore) -> None:
    ...
    # The key was just created in sign_content — it must be in the store now
...
def test_tampered_content_fails_verification(keystore: KeyStore) -> None:
    ...
    tampered_entry = SignedEntry(
        ...
        content_hash=entry.content_hash,   # original hash — will mismatch
...
def test_deterministic_keys_cross_keystore_verification() -> None:
    ...
    itself — so we test cross-keystore verification rather than byte equality.
    """
    ...
    # Materialise the same key in ks2 (same seed → same private/public key)
