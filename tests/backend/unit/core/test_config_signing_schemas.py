"""
Backend Unit Tests — core/config.py & core/signing.py & api/schemas.py
========================================================================
...
class TestSettings:
    ...
    def test_settings_importable(self):
        try:
            ...
            pytest.skip("backend.core.config not importable")
    ...
    def test_jwt_expire_minutes_is_reasonable(self):
        ...
        try:
            ...
            assert settings.jwt_access_token_expire_minutes <= 120, (
                f"JWT expires in {settings.jwt_access_token_expire_minutes} min — "
                "should be ≤ 120 (v1.0.3 fix)"
            )
        except ImportError:
            pytest.skip()
    ...
    def test_insecure_default_detected(self):
        ...
        try:
            ...
            if dev_placeholder in key:
                # In test env, this is fine — but the config should not force production mode with this key
                assert settings.app_env != "production", (
                    "Dev signing key detected in production mode — security violation"
                )
        except ImportError:
            pytest.skip()
...
class TestSignContent:
    """Tests for core/signing.py — ECDSA P-256 / SHA-256."""

    @pytest.fixture(autouse=True)
    def skip_if_missing(self):
        try:
            from backend.core.signing import sign_content, verify_entry, SignedEntry
            self.sign_content = sign_content
            self.verify_entry = verify_entry
            self.SignedEntry = SignedEntry
        except ImportError:
            pytest.skip("backend.core.signing not importable")
...
class TestSignedEntry:
    ...
    def skip_if_missing(self):
        try:
            from backend.core.signing import SignedEntry
            self.SignedEntry = SignedEntry
        except ImportError:
    ...
    def skip_if_missing(self):
        try:
            from backend.api.schemas import (
                InvestigationRequest, AgentFindingDTO, ReportDTO, HITLDecisionRequest,
            )
            ...
            pytest.skip("backend.api.schemas not importable")
