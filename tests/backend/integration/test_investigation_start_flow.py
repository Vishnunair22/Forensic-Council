...
async def test_run_investigation_task_awaits_final_report_cache(monkeypatch):
    ...
    monkeypatch.setattr(
        investigation_routes, "_wrap_pipeline_with_broadcasts", AsyncMock(return_value=report)
    )
    monkeypatch.setattr(investigation_routes, "get_session_websockets", lambda _sid: [])
    monkeypatch.setattr(investigation_routes, "broadcast_update", AsyncMock())
    monkeypatch.setattr(investigation_routes, "set_final_report", AsyncMock())
    monkeypatch.setattr(investigation_routes, "set_active_pipeline_metadata", AsyncMock())
    monkeypatch.setattr(investigation_routes, "increment_investigations_completed", lambda: None)
    monkeypatch.setattr(investigation_routes, "increment_investigations_failed", lambda: None)
    monkeypatch.setattr(investigation_routes.os.path, "exists", lambda _path: False)
    ...
    investigation_routes.set_final_report.assert_awaited_once()
