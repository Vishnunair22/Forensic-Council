...
def mock_pipeline_context():
    ...
    with patch("infra.redis_client.get_redis_client", new_callable=AsyncMock) as m_redis, \
         patch("infra.postgres_client.get_postgres_client", new_callable=AsyncMock) as m_pg, \
         patch("infra.qdrant_client.get_qdrant_client", new_callable=AsyncMock) as m_qd:
