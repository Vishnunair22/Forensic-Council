...
class TestAuthSecurity:
    ...
    def test_expired_token_rejected(self):
        ...
        expired_token = create_access_token(
            user_id="u1", role=UserRole.INVESTIGATOR, username="u1",
    ...
    def test_header_segment_tampering_rejected(self):
        ...
        bad_header = base64.urlsafe_b64encode(b'{"alg":"HS256","typ":"JWT","x":"injected"}').rstrip(b"=").decode()
...
class TestCryptographicSigning:
    ...
    def test_tampered_content_fails_verification(self):
        from core.signing import KeyStore, sign_content, verify_entry, SignedEntry
        ks = KeyStore()
        content = {"finding": "test", "confidence": 0.9}
        entry = sign_content("agent1_image", content, keystore=ks)
        # Tamper with the content hash (keep same signature — mismatch detectable)
    ...
    def test_different_keys_fail_verification(self):
        ...
        # ks2 has different keys — verification must fail
...
# ═══════════════════════════════════════════════════════════════════════════════
# INPUT VALIDATION — CASE ID FORMAT
# ═══════════════════════════════════════════════════════════════════════════════
...
class TestCustodyChainIntegrity:
    ...
    async def test_chain_writes_one_row_per_entry(self):
        """Each log_entry call must attempt a DB write — one INSERT per entry."""
