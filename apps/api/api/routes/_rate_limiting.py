...
async def check_investigation_rate_limit(user_id: str) -> None:
    ...
    try:
        ...
        if is_production:
            raise HTTPException(
                status_code=503,
                detail="Rate limiting service unavailable in production",
            )
...
async def check_daily_cost_quota(user_id: str, user_role: str = "investigator") -> None:
    ...
    try:
        ...
        logger.warning("Redis unavailable for cost quota", error=str(e))
        try:
            from api.routes.metrics import increment_rate_limit_redis_bypasses
        ...
        if is_production:
            raise HTTPException(
                status_code=503,
                detail="Cost quota service unavailable in production",
            )
