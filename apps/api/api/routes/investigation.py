...
async def start_investigation(
    ...
    tmp_path = Path(tempfile.gettempdir()) / f"{session_id}{raw_suffix}"
    ...
    try:
        ...
        if total_size == 0:
            tmp_path.unlink(missing_ok=True); raise HTTPException(status_code=400, detail="File is empty.")
        ...
        try:
            ...
            if _redis:
                was_set = await _redis.set(dedup_key, session_id, nx=True, ex=settings.investigation_timeout + 60)
                if not was_set:
        ...
        import magic
        with open(tmp_path, "rb") as _f: head = _f.read(2048)
        mime = magic.from_buffer(head, mime=True)
        valid_exts = _EXACT_MIME_EXT_MAP.get(mime, frozenset())
        if not valid_exts or raw_suffix not in valid_exts:
            tmp_path.unlink(missing_ok=True)
            raise HTTPException(status_code=400, detail=f"Security violation: content '{mime}' mismatch ext '{raw_suffix}'")

        if mime.startswith("image/") and mime != "image/gif":
            try:
                from PIL import Image
                with Image.open(str(tmp_path)) as img: img.verify()
                with Image.open(str(tmp_path)) as img2:
                    w, h = img2.size
                    if w * h > 100_000_000: raise HTTPException(status_code=400, detail="Image too large.")
            except HTTPException: raise
            except Exception: logger.warning("Image integrity check failed — corrupted?")

        pipeline = ForensicCouncilPipeline()
        set_active_pipeline(session_id, pipeline)
        task = asyncio.create_task(run_investigation_task(session_id=session_id, pipeline=pipeline, evidence_file_path=str(tmp_path), case_id=case_id, investigator_id=investigator_id, original_filename=file.filename))
        ...
        if settings.use_redis_worker:
            try:
                ...
                await get_investigation_queue().submit(session_id=UUID(session_id), case_id=case_id, investigator_id=investigator_id, evidence_file_path=str(tmp_path), original_filename=file.filename)
            except Exception: pass

        await set_active_pipeline_metadata(session_id, {"status": "running", "brief": "Initialising forensic pipeline...", "case_id": case_id, "investigator_id": investigator_id, "file_path": str(tmp_path), "original_filename": file.filename, "created_at": datetime.now(UTC).isoformat()})
        increment_investigations_started()
        ...
        tmp_path.unlink(missing_ok=True); raise HTTPException(status_code=500, detail=str(e))
