...
async def optical_flow_analyze(
    ...
    try:
        ...
        try:
            ...
            while True:
                ...
                flow = cv2.calcOpticalFlowFarneback(
                    prev_gray,
                    gray,
                    None,  # type: ignore[arg-type]  # cv2 stubs omit the flow output parameter
                    pyr_scale=0.5,
                    levels=3,
                    winsize=15,
                    iterations=3,
                ...
                if progress_callback and frame_idx % 25 == 0:
                    await progress_callback(
                        f"Processing motion vectors: frame {frame_idx}/{frame_count}..."
                    )
        ...
        if len(flow_magnitudes) > 0:
            ...
            for i, mag in enumerate(flow_magnitudes):
                if std_flow > 0:
                    ...
                    # Use a very generous threshold - we only want to catch extreme spikes
                    if z_score > flow_threshold:
                        flagged_frames.append(i + 1)  # +1 because we start from frame 1

        # Normalize heatmap
        if np.max(heatmap_accumulator) > 0:
            heatmap_normalized = (heatmap_accumulator / np.max(heatmap_accumulator) * 255).astype(
                np.uint8
            )
        else:
        ...
        if isinstance(e, ToolUnavailableError):
            raise
        raise ToolUnavailableError(f"Optical flow analysis failed: {str(e)}") from e
...
async def frame_window_extract(
    ...
    try:
        ...
        if isinstance(e, ToolUnavailableError):
            raise
        raise ToolUnavailableError(f"Frame window extraction failed: {str(e)}") from e
...
async def frame_consistency_analyze(
    ...
    try:
        ...
        for i, frame_file in enumerate(frame_files):
            ...
            if prev_frame is not None and prev_hist is not None and prev_edges is not None:
                # Compute histogram difference
                hist_diff = cv2.compareHist(prev_hist, hist, cv2.HISTCMP_BHATTACHARYYA)

                # Compute edge difference
                edge_diff = np.sum(np.abs(prev_edges.astype(float) - edges.astype(float))) / (
        ...
        if isinstance(e, ToolUnavailableError):
            raise
        raise ToolUnavailableError(f"Frame consistency analysis failed: {str(e)}") from e
...
async def face_swap_detect(
    ...
    try:
        ...
        face_cascade = cv2.CascadeClassifier(
            cv2.data.haarcascades + "haarcascade_frontalface_default.xml"  # type: ignore[attr-defined]  # cv2-stubs missing cv2.data
        )
        ...
        if isinstance(e, ToolUnavailableError):
            raise
        raise ToolUnavailableError(f"Face swap detection failed: {str(e)}") from e


async def video_metadata_extract(artifact: EvidenceArtifact) -> dict[str, Any]:
    """
    ...
    try:
        ...
        if isinstance(e, ToolUnavailableError):
            raise
        raise ToolUnavailableError(f"Video metadata extraction failed: {str(e)}") from e
...
async def face_swap_detect_deepface(
    ...
    try:
            ...
            while True:
                ...
                if frame_idx % max(1, int(fps * 0.5)) == 0:
                    try:
                        ...
                        logger.debug(f"Failed to process face sample: {str(e)}")
                frame_idx += 1
        finally:
            cap.release()
        ...
        confidence = min(
            ...
            + (max(d["cosine_distance"] for d in discontinuities) if discontinuities else 0.0)
            * 0.5,
        )
