    ...
    "Agent1": [
        {
            ...
            "tools": ["ela_full_image", "ela_anomaly_classify", "jpeg_ghost_detect", "noise_fingerprint"],
    ...
    "Agent4": [
        {
            "id": "temporal",
            "label": "Temporal Flow",
            "tools": ["optical_flow_analyze", "frame_consistency_analysis", "interframe_forgery_detector"],
            "desc": "Frame-to-frame flow and motion-ghosting forgery detection.",
        },
        {
            "id": "biometric",
            "label": "Biometric Forgery",
            "tools": ["face_swap_detection", "rppg_liveness_check"],
            "desc": "DeepFace face-swap detection and remote PPG biometric liveness.",
        },
        {
            "id": "device",
    ...
    "Agent5": [
        {
            ...
            "tools": ["exif_extract", "extract_deep_metadata", "metadata_anomaly_scorer", "timestamp_analysis", "gps_timezone_validate"],
            "desc": "EXIF/XMP integrity and probabilistic fabrication detection.",
        },
        {
            "id": "binary_sig",
            "label": "Binary Signatures",
            "tools": ["file_hash_verify", "file_structure_analysis", "hex_signature_scan", "c2pa_validator"],
...
class SynthesisService:
    ...
    async def synthesize_findings(
        ...
        if "Agent1" in agent_id: agent_key = "Agent1"
        elif "Agent2" in agent_id: agent_key = "Agent2"
        elif "Agent3" in agent_id: agent_key = "Agent3"
        elif "Agent4" in agent_id: agent_key = "Agent4"
        elif "Agent5" in agent_id: agent_key = "Agent5"
        ...
        for grp in tool_groups:
            grp_findings = [f for f in target_findings if f.metadata.get("tool_name", "") in grp["tools"]]
            if not grp_findings: continue
...
"""
        try:
            ...
            if pre_confidence < 0.5 or pre_error_rate > 0.4: fallback_verdict = "SUSPICIOUS"
            elif pre_confidence < 0.7 or pre_error_rate > 0.2: fallback_verdict = "INCONCLUSIVE"
            ...
            return {
                ...
                "sections": [{"id": g["id"], "label": g["label"], "opinion": "Tool results available in raw findings list.", "severity": "MEDIUM"} for g in grouped_sections_data]
            }

    def _compact_metrics(self, f: AgentFinding) -> dict[str, Any]:
        _SKIP_META = {"tool_name", "stub_warning", "llm_synthesis", "llm_reasoning", "synthesis_phase", "analysis_phase", "analysis_source", "backend"}
        out = {}
        for k, v in f.metadata.items():
            if k in _SKIP_META: continue
