...
class QdrantClient:
    ...
    Usage:
        async with QdrantClient() as client:
            await client.create_collection("my_collection", vector_size=512)
    ...
    async def connect(self) -> None:
        ...
        try:
            ...
            logger.error("Failed to connect to Qdrant", error=str(e), exc_info=True)
    ...
    async def batch_upsert(
        self,
        collection_name: str,
        points: list[tuple[str | UUID, list[float], dict[str, Any] | None]],
    ) -> bool:
        """
