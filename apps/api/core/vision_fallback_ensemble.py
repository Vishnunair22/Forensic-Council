...
async def analyze_local_ensemble(
    file_path: str,
    exif_summary: dict[str, Any] | None = None,
    is_screen_capture_like: bool = False,
) -> GeminiVisionFinding:
    """
    Perform localized CPU-only forensic ensemble analysis as a fallback.
    ...
    # 1. Create dummy artifact for tools
    art = EvidenceArtifact.create_root(
        artifact_type=ArtifactType.ORIGINAL,
        file_path=file_path,
        content_hash="local_ensemble_fallback_hash",
        action="local_ensemble_analysis",
        agent_id="local_ensemble",
        session_id=uuid4(),
    )

    # Import tools dynamically to avoid circular references and startup bloat
    ...
    desc_parts = [
        f"Local forensic ensemble analysis of {file_path}.",
    ...
    # 6. Screen capture / digital UI checks
    ...
    if is_screenshot:
        ...
        # Scan OCR lines for platform indicators
    ...
    finding = GeminiVisionFinding(
        ...
        _contextual_narrative="Local analysis of evidence file contents using CLIP and DETR models.",
