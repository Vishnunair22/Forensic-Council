...
class Settings(BaseSettings):
    ...
    evidence_storage_path: str = Field(
        default=str(_APP_ROOT / "storage" / "evidence"), description="Path for evidence storage"
    )
    calibration_models_path: str = Field(
        default=str(_APP_ROOT / "storage" / "calibration_models"),
    ...
    # ── Sub-model accessors for navigability ─────────────────────────────────
    # These return lightweight grouped views.  Env vars and validators stay on
    # the root class — these are read-only projections for downstream code that
    # prefers `settings.llm.provider` over `settings.llm_provider`.

    @property
    def llm(self) -> "LLMGroup":
        return LLMGroup(
            ...
            enable_react_reasoning=self.llm_enable_react_reasoning,
            enable_post_synthesis=self.llm_enable_post_synthesis,
            gemini_text_calls_enabled=self.gemini_text_calls_enabled,
            arbiter_provider=self.arbiter_llm_provider,
            arbiter_api_key=self.arbiter_llm_api_key,
            arbiter_model=self.arbiter_primary_model,
            arbiter_fallback_chain=self.arbiter_fallback_chain,
            vision_chain=self.vision_provider_chain,
            text_chain=self.text_provider_chain,
            free_tier_mode=self.free_tier_mode,
        )

    @property
    def gemini(self) -> "GeminiGroup":
        return GeminiGroup(
            api_key=self.gemini_api_key,
            model=self.gemini_model,
            fallback_models=self.gemini_fallback_models,
            timeout=self.gemini_timeout,
            max_concurrent=self.gemini_max_concurrent,
            api_key_policy_ok=self.gemini_api_key_policy_ok,
            rpm_limit=self.gemini_rpm_limit,
            rpd_limit=self.gemini_rpd_limit,
            arbiter_api_key=self.arbiter_gemini_api_key,
        )

    @property
    def db(self) -> "DatabaseGroup":
        return DatabaseGroup(
            ...
            postgres_min_pool=self.postgres_min_pool_size,
            postgres_max_pool=self.postgres_max_pool_size,
            postgres_connect_timeout=self.postgres_connect_timeout,
            postgres_command_timeout=self.postgres_command_timeout,
            ...
            qdrant_https=self.qdrant_https,
            retry_max=self.database_retry_max,
            retry_delay=self.database_retry_delay,
            use_worker=self.use_redis_worker,
    ...
    def storage(self) -> "StorageGroup":
        return StorageGroup(
            evidence_path=self.evidence_storage_path,
            calibration_models_path=self.calibration_models_path,
            retention_days=self.evidence_retention_days,
        )

    @property
    def ml_models(self) -> "MLModelGroup":
        return MLModelGroup(
            hf_home=self.hf_home,
            torch_home=self.torch_home,
            yolo_model_dir=self.yolo_model_dir,
            yolo_model_name=self.yolo_model_name,
            enable_agpl=self.enable_agpl_models,
            enable_research=self.enable_research_models,
            siglip_model=self.siglip_model_name,
            aasist_model=self.aasist_model_name,
            voice_clone_model=self.voice_clone_model_name,
            easyocr_dir=self.easyocr_model_dir,
            numba_cache_dir=self.numba_cache_dir,
        )

    @property
    def agents(self) -> "AgentGroup":
        return AgentGroup(
            ...
            hitl_decision_timeout=self.hitl_decision_timeout,
            max_retries=self.investigation_max_retries,
            retry_delay=self.investigation_retry_delay,
            session_ttl_hours=self.session_ttl_hours,
            max_parallel_heavy_tools=self.max_parallel_heavy_tools,
            daily_cost_quota=self.daily_cost_quota_usd,
            daily_cost_quota_admin=self.daily_cost_quota_admin_usd,
            context_wait_timeout=self.agent_context_wait_timeout,
            ocr_timeout=self.ocr_tool_timeout,
            clip_timeout=self.clip_analysis_timeout,
            ml_subprocess_timeout=self.ml_subprocess_timeout_s,
            external_api_retry_max=self.external_api_retry_max,
            external_api_retry_delay=self.external_api_retry_delay,
        )
