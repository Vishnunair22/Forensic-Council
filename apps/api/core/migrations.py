...
MIGRATIONS: list[Migration] = [
    ...
    Migration(
        version=7,
        name="create_pipeline_traces_table",
        description="Create table for forensic hierarchical pipeline tracing",
        sql="""
        CREATE TABLE IF NOT EXISTS pipeline_traces (
            ...
            metadata JSONB,
            start_time_utc TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
            end_time_utc TIMESTAMP WITH TIME ZONE,
            duration_ms INTEGER
        );
        CREATE INDEX IF NOT EXISTS idx_traces_session ON pipeline_traces(session_id);
        CREATE INDEX IF NOT EXISTS idx_traces_parent ON pipeline_traces(parent_id);
        """,
        rollback_sql="""
        DROP TABLE IF EXISTS pipeline_traces;
        """,
        validation_sql="SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'pipeline_traces')",
    ),
]
...
class MigrationManager:
    ...
    async def validate_migration(self, migration: Migration, conn: Any | None = None) -> bool:
    ...
    async def migrate(self, target_version: int | None = None) -> bool:
        """
    ...
    async def status(self) -> dict:
        ...
        try:
            ...
            # Only disconnect if we truly own a private, non-singleton connection.
            # Singleton pools should only be closed via close_postgres_client().
