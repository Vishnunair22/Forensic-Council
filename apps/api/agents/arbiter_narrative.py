...
def _deduplicate_findings(findings: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for finding in canonical_findings.values():
        ...
        if not fingerprint or evidence_verdict_of(finding) == "NEGATIVE":
            deduped.append(finding)
        elif fingerprint not in seen_reasoning:
            deduped.append(finding)
...
- reliability_note: ≤20 words. Cite confidence %, error rate, and note if any tools used fallbacks."""
    ...
    async def _llm_arbiter_synthesis(
        ...
        layer5 = (
            ...
            "   - Use EXACT system verdicts (AUTHENTIC, SUSPICIOUS, LIKELY_MANIPULATED, MANIPULATED, INCONCLUSIVE). Do not substitute with synonyms like 'fake' or 'genuine'.\n\n"
    ...
    async def deliberate_narratives(
        ...
        _provider_str = str(getattr(self.config, "llm_provider", "none")).lower()
        if not use_llm or _provider_str == "none" or not active_agent_results:
            logger.info(
            ...
            raw = self._postprocess_narratives(
                ...
                False, ["Narrative generated from templates (LLM disabled)"],
                per_agent_metrics, all_findings, overall_verdict,
                analysis_coverage_note, "", "", shared_visual_context=_shared_vctx,
            )
