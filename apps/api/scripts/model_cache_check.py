...
def check_filesystem_cache() -> tuple[int, int]:
    ...
    for name, dir_str in CACHE_DIRS.items():
        ...
        if not path.exists():
            print(
                f"  {YELLOW}WARN{RESET}  {name:<16}  {dir_str}  ->  directory missing (volume not mounted?)"
            )
            continue
        ...
        if count >= min_f and (min_f == 0 or size_mb > 0.5):
            status = f"{GREEN}OK CACHED{RESET}"
            populated += 1
        elif min_f == 0:
            status = f"{CYAN}OK EMPTY{RESET} (populated at runtime)"
            populated += 1  # empty-ok dirs count as healthy
        else:
            status = f"{YELLOW}WARN EMPTY{RESET} (will download on first use)"
...
def check_specific_model_assets() -> bool:
    ...
    object_model_name = os.environ.get("YOLO_MODEL_NAME", settings.yolo_model_name)
    use_agpl_yolo = "yolo" in object_model_name.lower() and settings.enable_agpl_models
    if use_agpl_yolo:
        yolo_path = Path(settings.yolo_model_dir) / object_model_name
        ...
        print(f"  {GREEN}[OK]{RESET}  Object detector: DETR fallback active")
...
def soft_warmup() -> bool:
    ...
    if importlib.util.find_spec("magic") is not None:
        print(f"  {GREEN}OK{RESET}  python-magic (libmagic)")
    else:
        print(f"  {RED}FAIL{RESET}  python-magic (libmagic)  ->  module not found")
