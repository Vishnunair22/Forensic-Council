...
export function TribunalMatrix({ report }: TribunalMatrixProps) {
 ...
 const resolved = (
  ...
  return (
   <div className={clsx(
    "rounded-2xl overflow-hidden glass-panel border",
    hasContested ? "border-rose-500/20 bg-rose-500/[0.01]" : "border-amber-500/10 bg-amber-500/[0.01]"
   )}>
    <div className={clsx(
     "px-5 py-3.5 border-b border-white/[0.05] bg-white/[0.02] flex items-center justify-between",
    )}>
   ...
   <div className="p-5 space-y-4">
     ...
     <div className="space-y-3 border-l border-white/[0.05] pl-4">
      ...
      <div className="space-y-2">
       {resolved.length === 0 ? (
        ...
        resolved.map((f, i) => (
         <div
          key={f.resolution ?? f.plain_description ?? i}
         ...
         >
          <div className="flex items-center justify-between relative z-10">
           <span className="text-[10px] font-mono font-black text-emerald-400 tracking-tighter whitespace-nowrap">
            Resolution Applied
           </span>
    ...
    <div className="mt-4 flex items-center gap-4 text-[10px] font-mono text-foreground/30 border-t border-white/[0.03] pt-4">
