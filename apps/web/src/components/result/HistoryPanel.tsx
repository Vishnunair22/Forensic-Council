        ...
        <div className="flex items-center justify-between px-10 py-8 border-b border-white/5">
          <div className="flex flex-col gap-1">
             <h3 className="text-xl font-bold text-white flex items-center gap-3">
               <HistoryIcon className="w-5 h-5 text-cyan-500/50" />
          ...
          <div className="flex items-center gap-6">
            {history.length > 0 && (
              <button
                onClick={clearAll}
                className="group flex items-center gap-2 px-4 py-2 rounded-full text-[10px] font-bold text-rose-500/40 hover:text-rose-400 hover:bg-rose-500/5 transition-all outline-none"
              >
            ...
            <button 
              onClick={onDismiss} 
              className="text-[10px] font-bold tracking-widest text-white/40 hover:text-cyan-400 transition-colors"
            >
              Back to Analysis
            </button>
          </div>
          ...
          {history.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-24 gap-6 text-white/10">
              ...
              <div className="text-center">
                <p className="text-sm font-bold tracking-widest mb-2">Archive Empty</p>
              ...
              {history.sort((a,b) => b.timestamp - a.timestamp).map((item) => (
                ...
                >
                  ...
                  <div className="flex flex-wrap items-center gap-x-8 gap-y-3 pt-5 border-t border-white/[0.03]">
                    ...
                    <span className="text-[10px] font-bold tracking-widest text-white/30 px-2 py-0.5 rounded-full border border-white/5 bg-white/[0.02]">
                      {item.type} Analysis
                    </span>
                  </div>
                </div>
