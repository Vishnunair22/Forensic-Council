...
This design system is enforced mechanically via ESLint and Stylelint. Human PR reviews are insufficient to prevent UI drift. Any violation of these rules will fail the CI/CD pipeline.

### 2.1 ESLint Strict Utility Constraints
The following rules MUST be active in `eslint.config.mjs` to physically prevent developers from writing code that breaks the aesthetic or accessibility guidelines:
...
Forced uppercase is banned. Use Title Case for labels.
...
Frosted glass and dark themes fail in PDF export engines. When generating reports, the app sets a `data-print-mode="true"` attribute on the root `html`.

When active (handled globally in `globals.css`):
- All `fc-surface-*` elements drop their `backdrop-filter` and become `#ffffff`.
- All text classes invert to dark grays/blacks.
- All shadows are flattened to 1px solid gray borders.
- Registry blue darkens to `#1d4ed8` for high-contrast white paper readability.
...
- [ ] No ESLint or Stylelint warnings regarding Tailwind classes.
