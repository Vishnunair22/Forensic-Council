"use client";

import { useRef, useMemo, useEffect } from "react";
import Image from "next/image";
import { 
  RotateCcw, 
  ArrowRight, 
  UploadCloud, 
  FileAudio, 
  ShieldAlert, 
  ScanLine 
} from "lucide-react";
import { motion, AnimatePresence } from "framer-motion";
import { clsx } from "clsx";

interface FileUploadSectionProps {
  file: File | null;
  isDragging: boolean;
  isUploading: boolean;
  validationError: string | null;
  onFileSelect: (file: File) => void;
  onFileDrop: (file: File) => void;
  onDragEnter: () => void;
  onDragLeave: () => void;
  onUpload: (file: File) => void;
  onClear: () => void;
}

export function FileUploadSection({
  file,
  isDragging,
  isUploading,
  validationError,
  onFileSelect,
  onFileDrop,
  onDragEnter,
  onDragLeave,
  onUpload,
  onClear,
}: FileUploadSectionProps) {
  const fileInputRef = useRef<HTMLInputElement>(null);

  const filePreviewUrl = useMemo(() => {
    if (!file) return null;
    if (file.type.startsWith("image/") || file.type.startsWith("video/")) {
      return URL.createObjectURL(file);
    }
    return null;
  }, [file]);

  useEffect(() => {
    return () => {
      if (filePreviewUrl) URL.revokeObjectURL(filePreviewUrl);
    };
  }, [filePreviewUrl]);

  return (
    <div className="flex flex-col items-center justify-center min-h-[75vh] w-full max-w-2xl mx-auto px-4">
      {/* Title Section */}
      <motion.div 
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        className="text-center mb-12"
      >
        <div className="inline-flex items-center gap-3 px-4 py-2 rounded-full mb-8 bg-[#14b8a6]/5 border border-[#14b8a6]/10">
          <span className="relative flex h-2 w-2">
            <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-[#14b8a6] opacity-60" />
            <span className="relative inline-flex rounded-full h-full w-full bg-[#14b8a6]" />
          </span>
          <span className="uppercase tracking-[0.4em] font-black text-[9px] text-[#14b8a6]/80 font-mono">
            Evidence Intake Portal
          </span>
        </div>

        <h1 className="text-4xl md:text-5xl font-black mb-4 tracking-tighter text-white leading-tight uppercase font-heading">
          Initiate Investigation
        </h1>

        <p className="text-white/30 text-[10px] font-bold uppercase tracking-[0.25em] max-w-sm mx-auto leading-relaxed font-mono">
          High-Precision Forensic Auditing
        </p>
      </motion.div>

      {/* File Area */}
      <AnimatePresence mode="wait">
        {file ? (
          <motion.div
            key="preview"
            initial={{ opacity: 0, scale: 0.98 }}
            animate={{ opacity: 1, scale: 1 }}
            exit={{ opacity: 0, scale: 0.98 }}
            className="w-full rounded-3xl overflow-hidden glass-panel border-white/10 group shadow-[0_32px_64px_rgba(0,0,0,0.4)]"
          >
            {/* Preview Viewport */}
            <div className="relative w-full aspect-video bg-black/40 overflow-hidden">
              {file.type.startsWith("image/") && filePreviewUrl && (
                <div className="relative w-full h-full">
                  <Image
                    src={filePreviewUrl}
                    alt={file.name}
                    fill
                    className="object-contain"
                    unoptimized={true} // Local Blob URLs require unoptimized flag in some Next.js configs
                  />
                </div>
              )}
              {file.type.startsWith("video/") && (
                <video
                  src={filePreviewUrl ?? ""}
                  className="w-full h-full object-contain"
                  muted autoPlay loop playsInline
                />
              )}
              {!file.type.startsWith("image/") && !file.type.startsWith("video/") && (
                <div className="flex flex-col items-center justify-center h-full gap-4">
                  <div className="w-20 h-20 rounded-2xl flex items-center justify-center bg-white/5 border border-white/10">
                    <FileAudio className="w-10 h-10 text-[#14b8a6]" />
                  </div>
                </div>
              )}

              {/* Overlay Metadata */}
              <div className="absolute bottom-0 inset-x-0 p-6 bg-gradient-to-t from-black/80 to-transparent z-10">
                <div className="flex items-center justify-between">
                  <div className="flex flex-col">
                    <span className="text-xs font-bold text-white font-mono truncate max-w-xs">{file.name}</span>
                    <span className="text-[10px] text-white/40 uppercase font-bold tracking-widest mt-1">
                      {(file.size / 1024 / 1024).toFixed(2)} MB · {(file.type.split('/')[1] ?? file.type).toUpperCase()}
                    </span>
                  </div>
                </div>
              </div>
            </div>

            {/* Actions */}
            <div className="p-6 grid grid-cols-2 gap-4 bg-white/[0.02]">
              <button
                onClick={onClear}
                disabled={isUploading}
                className="btn-pill-secondary py-4 text-[10px] gap-2 border-white/5 active:scale-95"
              >
                <RotateCcw className="w-3.5 h-3.5" />
                Discard
              </button>
              <button
                onClick={() => onUpload(file)}
                disabled={isUploading}
                className="btn-pill-primary py-4 text-[10px] gap-3 active:scale-95"
              >
                {isUploading ? (
                  <>
                    <UploadCloud className="w-4 h-4 animate-bounce" />
                    Sealing...
                  </>
                ) : (
                  <>
                    <ScanLine className="w-4 h-4" />
                    Begin Audit
                    <ArrowRight className="w-3.5 h-3.5" />
                  </>
                )}
              </button>
            </div>
          </motion.div>
        ) : (
          <motion.div
            key="dropzone"
            initial={{ opacity: 0, y: 10 }}
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: -10 }}
            role="button"
            tabIndex={0}
            className={clsx(
              "w-full rounded-3xl transition-all duration-500 cursor-pointer overflow-hidden relative border-2 border-dashed group",
              isDragging ? "bg-cyan-500/5 border-cyan-500/40 scale-[1.02]" : "bg-white/[0.01] border-white/5 hover:border-white/20 hover:bg-white/[0.02]"
            )}
            onClick={() => fileInputRef.current?.click()}
            onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); fileInputRef.current?.click(); } }}
            onDragEnter={(e) => { e.preventDefault(); onDragEnter(); }}
            onDragOver={(e) => { e.preventDefault(); if (!isDragging) onDragEnter(); }}
            onDragLeave={onDragLeave}
            onDrop={(e) => {
              e.preventDefault();
              onDragLeave();
              const f = e.dataTransfer.files?.[0];
              if (f) onFileDrop(f);
            }}
          >
            <div className="py-20 flex flex-col items-center gap-6">
              <div className={clsx(
                "w-20 h-20 rounded-3xl flex items-center justify-center transition-all duration-500 border shadow-2xl",
                isDragging ? "bg-cyan-500/10 border-cyan-500/30 text-cyan-400 rotate-12" : "bg-white/5 border-white/10 text-white/20 group-hover:text-cyan-400/60 transition-transform group-hover:scale-110"
              )}>
                <UploadCloud className="w-8 h-8" strokeWidth={1} />
              </div>

              <div className="text-center space-y-1">
                <p className="text-lg font-black text-white/90 tracking-tight group-hover:text-white transition-colors uppercase font-heading">
                  {isDragging ? "Finalizing selection" : "Identify Evidence"}
                </p>
                <p className="text-[11px] font-mono font-bold text-white/20 tracking-widest uppercase">
                  Drag & Drop or Click to Browse
                </p>
              </div>

              <div className="flex gap-3 mt-4">
                {["IMG", "VID", "AUD"].map((t) => (
                  <span key={t} className="px-3 py-1 rounded bg-white/[0.02] border border-white/[0.05] text-[9px] font-black text-white/20 font-mono">
                    {t}
                  </span>
                ))}
              </div>
            </div>
          </motion.div>
        )}
      </AnimatePresence>

      {/* Validation Error */}
      <AnimatePresence>
        {validationError && (
          <motion.div
            initial={{ opacity: 0, y: 10 }}
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, scale: 0.95 }}
            className="mt-8 p-5 rounded-2xl glass-panel border-rose-500/20 bg-rose-500/[0.04] flex items-start gap-4 max-w-md"
          >
            <ShieldAlert className="w-6 h-6 text-rose-500 shrink-0 mt-0.5" />
            <div className="space-y-1">
              <h4 className="text-xs font-black text-rose-400 uppercase tracking-widest">Protocol Violation</h4>
              <p className="text-xs text-rose-200/60 font-medium leading-relaxed">{validationError}</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>

      <input
        ref={fileInputRef}
        type="file"
        onChange={(e) => {
          const f = e.target.files?.[0];
          if (f) onFileSelect(f);
        }}
        className="sr-only"
        accept="image/*,video/*,audio/*"
      />
    </div>
  );
}
