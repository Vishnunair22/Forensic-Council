...
export function ForensicTimeline({ 
  ...
  const flaggedFindings = allFindings.filter(f => f.verdict === "FLAGGED" || f.severity === "CRITICAL" || f.severity === "HIGH");
    ...
    <div className="w-full h-full flex flex-col gap-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h4 className="text-xs font-black uppercase tracking-widest text-white/50 leading-none mb-1">Evidence Ledger</h4>
            <span className="text-[10px] font-mono text-white/20 uppercase tracking-tighter">Real-time Forensic Feed</span>
          </div>
        </div>
        ...
        <div className="flex gap-2 items-center">
           <motion.div 
             ...
             className="px-2 py-0.5 rounded-full bg-cyan-500/10 border border-cyan-500/30 text-cyan-400 text-[8px] font-black uppercase tracking-widest flex items-center gap-1.5"
           >
             <span className="w-1.5 h-1.5 rounded-full bg-cyan-400" />
             LIVE_FEED
           </motion.div>
           <div className="px-2 py-0.5 rounded-full bg-rose-500/10 border border-rose-500/20 text-rose-400 text-[9px] font-black uppercase">
          ...
          <AnimatePresence mode="popLayout">
            {isInitialating && (
              ...
              >
                ...
                <div className="flex-1 pt-1.5 min-w-0">
                   <p className="text-[11px] font-mono text-cyan-400 leading-relaxed uppercase font-black tracking-widest">
                     Establishing Secure Pipe
                   </p>
                   <span className="text-[9px] text-white/20 uppercase">Awaiting Stream Initialization</span>
                </div>
              </motion.div>
            )}
            ...
            {allFindings.length > 0 ? (
              allFindings.map((finding, idx) => {
                ...
                const isAlert = finding.verdict === "FLAGGED" || finding.severity === "CRITICAL" || finding.severity === "HIGH";
                  ...
                  >
                    ...
                    >
                      ...
                      <div className={clsx(
                        ...
                        isAlert ? "bg-rose-500/20 border-rose-500/30 text-rose-400 shadow-[0_0_10px_rgba(244,63,94,0.2)]" : "bg-white/5 border-white/10 text-white/30"
                      )}>
              ...
              >
                ...
                <p className="text-[10px] font-black uppercase tracking-[0.3em]">No Intelligence Logged</p>
              </motion.div>
            )}
          </AnimatePresence>
        </div>
      ...
      <div className="mt-auto border-t border-white/5 pt-4 flex items-center justify-between">
         <div className="flex items-center gap-1.5">
           ...
           <span className="text-[9px] font-black uppercase tracking-widest text-white/20">Live Intelligence Feed</span>
         </div>
         <span className="text-[9px] font-mono text-white/10 font-black">
