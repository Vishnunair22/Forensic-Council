...
interface GroundPlainInterface {
  ...
  moveNoise: (mouse: { x: number; y: number }) => void;
...
const AnimatedWave: React.FC<AnimatedWaveProps> = ({
  ...
  const sceneElementsRef = useRef<{
    ...
    groundPlain: GroundPlainInterface | null;
  ...
  const createGroundPlain = useCallback(() => {
    ...
    const groundPlain = {
        ...
        for (let i = 0; i < positions.count; i++) {
          ...
          if (mouseInteraction && this.mouseDistortionStrength > 0) {
            ...
            const mouseRippleNoise = this.simplex(
              distX_mouse / this.mouseDistortionSmoothness + this.distortionTime, // Offset by time for evolution
      ...
      dispose: function () {
        ...
        if (this.plane) this.group?.remove(this.plane);
        this.plane = null;
        this.geometry = null;
        this.material = null;
        this.simplex = null;
  ...
  const setupScene = useCallback(() => {
    ...
    return () => {
      ...
      if (current) {
        ...
        if (current.renderer && containerRef.current?.contains(current.renderer.domElement)) {
