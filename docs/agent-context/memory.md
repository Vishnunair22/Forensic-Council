...
Groq's hardware inference is significantly faster for the ReAct loop reasoning steps, which need low latency per iteration. Gemini is used specifically for vision tasks (deep pass) where Groq has no native vision support. As of v1.4.0, we use **Llama 3.3 70b** on Groq for orchestration.
...
Default was `gemini-3-pro-preview` (not a real model). Corrected to `gemini-2.5-flash` in v1.4.0 for primary stability, with `gemini-2.0-flash` series as fallbacks.
...
8. `GEMINI_API_KEY` set — use `gemini-2.5-flash` model as primary with `gemini-2.0-flash` as secondary.
