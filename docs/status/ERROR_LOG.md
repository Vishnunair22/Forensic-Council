...
### 🔴 BUG 10 — CRITICAL: EvidenceStoreError on Upload (Postgres JSONB & Missing Table)
**Files:** `backend/infra/postgres_client.py`, `backend/scripts/init_db.py`
**Problem:** When a user uploads a file, the system failed with a silent `DataError` wrapped as a generic `EvidenceStoreError`. This was caused by two compounding issues:
1. The `asyncpg` library frequently fails to automatically infer the JSON codec for standard Python dictionaries during `INSERT` queries unless they are pre-serialized to JSON strings.
2. After patching the serialization, the deeper error `UndefinedTableError('relation "evidence_artifacts" does not exist')` was revealed because the `evidence_artifacts` table was missing from the initial PostgreSQL database bootstrap.
**Fix:** 
1. Updated the `execute`, `execute_many`, `fetch`, `fetch_one`, and `fetch_val` methods in `PostgresClient` to automatically convert `dict` arguments to standard JSON strings (`json.dumps()`).
2. Ran `docker compose exec backend python scripts/init_db.py` to create the missing `evidence_artifacts` table in the running database.
...
|---|---|---|---|---|
| 290 | **CORS policy blocks `fetch()` to `localhost:8000` from `localhost:3000`** | 🔴 Critical | **RESOLVED** | Root cause: Starlette's `ServerErrorMiddleware` (outermost) can catch exceptions thrown in user-defined `@app.middleware("http")` handlers before `CORSMiddleware` (innermost user middleware) gets to stamp `Access-Control-Allow-Origin` on the response — browser never sees the header and blocks the request. **Fix**: Added `async rewrites()` in `frontend/next.config.ts` that proxies all `/api/v1/*` requests through the Next.js server to the backend via `INTERNAL_API_URL` (Docker-internal hostname). Changed `API_BASE` in `frontend/src/lib/api.ts` to an empty string `""` when running in the browser so all API calls use relative URLs (`/api/v1/...`). The browser makes same-origin requests to Next.js; Next.js proxies server-to-server to the backend — no cross-origin request is ever made, CORS is bypassed entirely. WebSocket connections retain their absolute URL (browser WS upgrades are not subject to the same CORS enforcement as `fetch()`). |
| 291 | **`/api/v1/investigate` returns 500 — `write_temp_file` outside try block + MIME fallback missing + HTTPException swallowed by global handler** | 🔴 Critical | **RESOLVED** | Three compounding bugs: (1) `evidence_file_path = await asyncio.to_thread(write_temp_file, ...)` was called **outside** the route's `try/except` block — any OS-level failure (permissions, tmpfs) threw an unhandled exception, going straight to the global exception handler as a mystery 500. Fixed by moving `write_temp_file` inside the `try` block and initialising `evidence_file_path = None` before it. (2) `import magic` was called inline at function scope with no fallback — if `libmagic` is unavailable the inner `try/except` returns `""`, which fails the MIME allowlist check and returns 422 (not 500), but the code was fragile. Fixed by adding an `_EXT_MIME_MAP` extension-based fallback so MIME detection never silently returns empty. (3) The global `@app.exception_handler(Exception)` blindly returned `status_code=500` even when an `HTTPException` leaked through, hiding the real status code. Fixed by adding an `isinstance(exc, HTTPException)` guard that honours the original status code. (4) `api.ts` only read `error.detail` from backend error responses, silently dropping `error.message` (which contains the actual Python exception in dev mode). Fixed by appending `error.message` to the thrown error string so the real cause appears in the browser console. |
...
| 292 | **Agents disappear and UI freezes at Object Agent** | 🔴 Critical | **RESOLVED** | Root cause: `useSimulation.ts` was writing WebSocket callback data asynchronously using stale React `completedAgents` state values. If background agents (skipped) finished while an active agent was rendering, the array would be overwritten, effectively deleting the skipped agents from memory. This caused the completed agent count to never reach 5, so the pipeline `setStatus("complete")` never fired, freezing the app midway through. **Fix**: Replaced tracking with synchronous `completedAgentsRef.current.push()` mutations. |
| 293 | **`setStatus("analyzing")` never called — sequence progression blocked** | 🔴 Critical | **RESOLVED** | Root cause: When the old queue/lock system was removed from `useSimulation.ts`, the only call to `setStatus("analyzing")` was also deleted. `startSimulation()` sets status to `"initiating"`, but nothing ever transitioned it to `"analyzing"`. The sequence progression `useEffect` guarded on `status === "analyzing" \|\| status === "processing"` — since status stayed at `"initiating"`, the effect **never fired**. This caused: (1) Agent 1 card appears briefly because `uiSequenceIndex === 0`, (2) Agent 1 completes in the background but `uiSequenceIndex` never advances, (3) subsequent agents' AGENT_UPDATE messages update `agentUpdates` state but no new cards render since `uiSequenceIndex` stays at 0, (4) Object Agent's thinking text shows in the progress bar because it's the last AGENT_UPDATE received. **Fix**: Added `setStatus((prev) => prev === "initiating" ? "analyzing" : prev)` in both the AGENT_UPDATE and AGENT_COMPLETE handlers. Also widened the `useEffect` guard to include `"initiating"` status, and used `completedAgentsRef.current` instead of the async `completedAgents` state for completion checks. |
| 294 | **Analysis panel vanishes on backend error — agent cards disappear** | 🔴 Critical | **RESOLVED** | Root cause: `page.tsx` conditionally rendered the analysis section with `(status !== "idle" && status !== "error")`. When the backend pipeline crashes and broadcasts an `ERROR` WebSocket message, the frontend sets `status = "error"`, which triggers the condition to hide the entire analysis panel and switch back to the upload form — making all in-progress agent cards disappear. **Fix**: Added `hasStartedAnalysis` flag that becomes true once any `agentUpdates` or `completedAgents` exist. The analysis panel now stays visible when `hasStartedAnalysis` is true, even on error. Added inline error banner within the analysis section with "Try Again" button. |
| 295 | **Agent cards conditionally mounted via AnimatePresence — fragile** | 🔴 Critical | **RESOLVED** | Root cause: Agent cards were wrapped in `<AnimatePresence>{shouldRender && (<motion.div>...</motion.div>)}</AnimatePresence>`. The `shouldRender` condition depended on `uiSequenceIndex` which is driven by the sequence progression `useEffect`. Any race condition between state updates (completedAgents, status, uiSequenceIndex) could cause `shouldRender` to momentarily flip false, unmounting a card entirely. Combined with React 18's batching behavior, this made cards "disappear". **Fix**: Nuclear rewrite — removed ALL conditional AnimatePresence mounting. All 5 agent cards are ALWAYS rendered in the DOM from the moment analysis starts. Cards use three visual states via CSS: **pending** (35% opacity, shows description text), **active** (full opacity, amber border, shows streamed thinking text with pulse icon), **complete** (green border, shows results with confidence badge). Cards can never disappear because they are never unmounted. |
| 296 | **Pipeline stuck at Agent 3 — no per-agent timeout** | 🔴 Critical | **RESOLVED** | Root cause: `run_single_agent` used `asyncio.wait_for(agent.run_investigation(), timeout=pipeline.config.investigation_timeout)` — the FULL pipeline timeout (~120-300s). If Agent3's YOLOv8 model download hangs or the LLM call stalls, a single agent consumes the entire pipeline timeout, blocking all subsequent agents. Also, `asyncio.gather(*tasks)` propagated exceptions — one crashing agent aborted the whole pipeline. **Fix**: (1) Per-agent timeout reduced to `min(60, pipeline_timeout/2)`. (2) Explicit `asyncio.TimeoutError` catch in `run_single_agent`. (3) `asyncio.gather(*tasks, return_exceptions=True)` with fallback `AGENT_COMPLETE` broadcast for crashed agents so the frontend sequence always advances. |
| 297 | **`evidence_artifacts` table missing — migration failure at startup** | 🟠 High | **RESOLVED** | Root cause: If the database migration fails at startup (e.g., Postgres not ready, network issue), the server still starts but the `evidence_artifacts` table doesn't exist. The first `INSERT INTO evidence_artifacts(...)` in `evidence_store._save_artifact()` then throws `UndefinedTableError`, crashing the entire pipeline before any agents run. **Fix**: `_save_artifact()` now catches the `UndefinedTableError`, creates the table + indexes inline, and retries the insert. |
| 298 | **Agent timeout chain misconfigured — LLM retries exceed per-agent budget** | 🔴 Critical | **RESOLVED** | Root cause: `llm_timeout=30s` with `_MAX_RETRIES=3` meant a single failed LLM call could take ~93s (30+1+30+2+30). Per-agent timeout was 60s, so agents with unreachable LLM always timed out before even completing one ReAct iteration. Combined with 5 concurrent agents each making LLM calls, the pipeline stalled. **Fix**: (1) `llm_timeout` reduced 30→15s in `config.py`, (2) `_MAX_RETRIES` reduced 3→2 in `llm_client.py`, (3) per-agent timeout increased 60→90s in `investigation.py`. Worst-case single LLM failure = 15+1+15 = 31s, well within 90s budget. |
| 299 | **LLM failure kills ReAct loop — zero findings produced** | 🔴 Critical | **RESOLVED** | Root cause: In `react_loop.py`, the `llm_step_generator` catch block returned `None` on LLM API failure. But the main loop treated `None` as "all tasks complete" and broke immediately — producing zero findings and zero tool calls. This meant any LLM API issue (wrong key, network, rate limit) would cause all agents to return empty results silently. **Fix**: Changed the loop logic so when `llm_generator` returns `None`, the loop falls back to `_default_step_generator` (task-decomposition driver). This ensures agents always run their tools and produce real findings, just without LLM reasoning annotations. Only breaks when BOTH the LLM generator and the task-decomposition driver return `None`. |
| 300 | **UI appears frozen — zero progress updates during agent analysis** | 🔴 Critical | **RESOLVED** | Root cause: `run_single_agent` in `investigation.py` sent ONE `AGENT_UPDATE` at the start, then ran the full ReAct loop (10-20 iterations of LLM + tool calls) in complete silence — no WebSocket updates for potentially 60-180 seconds. Also, per-agent timeout was 90s which is too short for real LLM-driven analysis (each Groq call takes 1-5s, with 10-20 iterations = 60-100s minimum). **Fix**: (1) Added heartbeat progress streamer — a background `asyncio.Task` that runs alongside each agent, checks the agent's internal `_react_chain` every 8 seconds, and broadcasts the latest tool/thought as an `AGENT_UPDATE` via WebSocket. (2) Increased per-agent timeout from 90s to 180s (3 min). Frontend now receives live thinking updates like "Running tool: Ela Full Image...", "Analyzing JPEG Ghost Detection results...", etc. |
| 301 | **LLM in ReAct loop causes rate limiting and 180s+ analysis time** | 🔴 Critical | **RESOLVED** | Root cause: Groq (Llama 3.3 70B) was called on EVERY ReAct iteration — each agent made 10-20 LLM calls, totaling 60+ API calls across 5 agents. Groq's free tier limits to 30 req/min, causing 429 rate-limit errors, retries, and eventual timeouts. **Fix**: Restructured LLM usage — `llm_enable_react_reasoning` set to `False` by default. Agents now use the fast task-decomposition driver (no LLM calls) to run tools in ~15-30s. After all tools complete, Groq is called ONCE per agent via `_synthesize_findings_with_llm()` to enrich raw findings into court-grade forensic narratives. Total LLM calls reduced from 60+ to ~6. Arbiter's existing LLM usage (executive summary, uncertainty statement) unchanged. |

| 302 | **UI says 'Deep analysis in progress step 1', stuck there** | �x � Critical | **RESOLVED** | Root cause: Heartbeat was reading from `agent._react_chain` which is only populated *after* the ReAct loop finishes, so it always showed generic fallback text. Fix: Rewrote heartbeat to read live from `WorkingMemory.get_state()`. The loop engine now marks tasks `IN_PROGRESS` as they run. Heartbeat now shows real task names like "Running: ELA full-image analysis (3/13 complete)". |
| 303 | **180s Timeout persists even with fast task-decomposition driver** | �x � Critical | **RESOLVED** | Root cause: Two issues. (1) Task-decomposition driver wasted iterations by emitting THOUGHT then ACTION (2 iterations per task). 13 tasks = 26 iterations, but `iteration_ceiling` was 20. (2) Heavy ML tools (CLIP, SIFT, OCR) still take 15-30s each on CPU, easily exceeding 180s when run sequentially. Fix: Implemented **Two-Phase Analysis**. (1) Driver optimized to emit ACTION directly (1 iter/task). (2) Agent task lists split into `task_decomposition` (fast non-ML tools, ~15s) and `deep_task_decomposition` (slow ML tools). (3) Orchestration modified so agents run their fast pass, signal completion to the UI, then run their deep pass as a background coroutine while subsequent agents run. Arbiter waits for all deep passes before generating the final report. Perceived UI speed drastically improved while retaining full forensic depth. |
