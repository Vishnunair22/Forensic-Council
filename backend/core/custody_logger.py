...
_custody_retry_queue_size: int = 0


def get_custody_metrics() -> dict[str, int]:
    """Return custody logger health metrics for monitoring endpoints."""
    return {
        "write_failures": _custody_write_failures,
        "retry_queue_size": _custody_retry_queue_size,
    }
...
class CustodyLogger:
    ...
    async def log_entry(
        ...
        try:
            ...
            global _custody_write_failures, _custody_retry_queue_size
            ...
            _custody_retry_queue_size = len(self._retry_queue)
            return None
    ...
    async def _flush_retry_queue(self) -> None:
        ...
        if flushed > 0:
            logger.info(
                f"Flushed {flushed} queued custody entries, {len(still_queued)} remaining"
            )
