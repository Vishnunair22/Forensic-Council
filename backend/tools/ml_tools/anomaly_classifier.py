...
def classify_anomaly(image_path_a: str, image_path_b: str, 
                     motion_vector_magnitude: float = 0.0) -> dict:
    """
    ...
    is_suspicious = (
        motion_vector_magnitude < 5.0 and diff_area_ratio > 0.05 and ssim_score < 0.85
    ...
    if diff_area_ratio > 0.05 and motion_vector_magnitude < 5.0:
        reasons.append(f"Localized difference cluster ({diff_area_ratio:.1%} of frame) with low motion")
    if not reasons:
        reasons.append(f"High motion ({motion_vector_magnitude:.1f}px) explains structural difference")
