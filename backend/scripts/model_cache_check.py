...
# These must match the ENV vars and volume mounts in the Dockerfile / compose.
CACHE_DIRS: dict[str, str] = {
    "HuggingFace":     os.getenv("HF_HOME",           "/app/cache/huggingface"),
...
def check_filesystem_cache() -> tuple[int, int]:
    ...
    for name, dir_str in CACHE_DIRS.items():
        ...
        if not path.exists():
            print(f"  {YELLOW}⚠  {name:<16}{RESET}  {dir_str}  →  directory missing (volume not mounted?)")
            continue
...
def soft_warmup() -> None:
    ...
    if failed:
        ...
        print(f"  {YELLOW}Hint: Run `uv sync --frozen --extra ml` to reinstall dependencies.{RESET}")
    else:
...
def main() -> None:
    ...
    if populated == total:
        ...
        print(f"  {YELLOW}Expect slower first-run analysis; subsequent runs use the cache.{RESET}")
