...
async def run_stress_test():
    ...
    for i, img_path in enumerate(test_images, 1):
        ...
        case_id = f"STRESS-TEST-{uuid4().hex[:8]}"
        ...
        try:
            ...
            for res in agent_results:
                if res.error:
                    ...
                    tools_run = [f.get('metadata', {}).get('tool_name', 'Unknown') for f in res.findings]
            ...
            for aid in ["Agent1", "Agent3", "Agent5"]:
                ...
                gemini_found = any("gemini" in f.get('metadata', {}).get('tool_name', '').lower() for f in [f.model_dump() for f in findings])
                ...
                for f in findings:
                    ...
                    if "Gemini" in tool and (not desc or desc == "Visual analysis complete." or "unidentified content" in desc):
