    ...
    async def build_tool_registry(self) -> ToolRegistry:
        ...
        async def exif_extract_handler(input_data: dict) -> dict:
            ...
            try:
                ...
                result["file_name"] = artifact.metadata.get(
            ...
            if meta:
                ...
                # Timestamps
                dt_orig = meta.get(
                    "EXIF:DateTimeOriginal", meta.get("DateTimeOriginal", "")
                )
                dt_mod = meta.get(
                    "File:FileModifyDate", meta.get("DateTimeModified", "")
                )
                if dt_orig:
        ...
        async def gps_timezone_validate_handler(input_data: dict) -> dict:
            ...
            try:
                ...
                exif_result = self._tool_context.get(
                    "exif_extract"
                ) or await real_exif_extract(artifact=artifact)
                gps = exif_result.get("gps_coordinates")
                if not gps:
                    return {
                        "plausible": None,
                        "issues": ["No GPS data present in file"],
                        "timezone": "N/A",
                    }
                ...
                if not ts:
                    return {
                        "plausible": None,
                        "issues": ["No timestamp in EXIF"],
                        "timezone": "N/A",
                    }

                ts_iso = (
                    ts.replace(":", "-", 2).replace(" ", "T") + "Z"
                    if "T" not in ts
                    else ts
                )
        ...
        async def steganography_scan_handler(input_data: dict) -> dict:
            ...
            if (
                any(s in software for s in ("gimp", "photoshop", "steg", "hide"))
                or not software
            ):
            ...
            try:
                ...
                logger.debug(f"LSB decode failed in steganography_scan_handler: {e}")
        ...
        async def file_structure_analysis_handler(input_data: dict) -> dict:
            ...
            if result.get("error"):
                await self._record_tool_error(
                    "file_structure_analysis", result["error"]
                )
            else:
        ...
        async def file_hash_verify_handler(input_data: dict) -> dict:
            ...
            if evidence_store is None:
                ...
                result = await real_file_hash_verify(
                    artifact=artifact, evidence_store=evidence_store
                )
            if result.get("error"):
        ...
        async def astronomical_api_handler(input_data: dict) -> dict:
            ...
            try:
                ...
                exif_result = (
                    self._tool_context.get("exif_extract")
                ...
                loc = LocationInfo(latitude=gps["latitude"], longitude=gps["longitude"])
        ...
        async def reverse_image_search_handler(input_data: dict) -> dict:
            ...
            try:
                ...
                candidate_paths = (
                    glob.glob(os.path.join(evidence_dir, "**", "*.jpg"), recursive=True)
                    + glob.glob(
                        os.path.join(evidence_dir, "**", "*.jpeg"), recursive=True
                    )
                    + glob.glob(
                        os.path.join(evidence_dir, "**", "*.png"), recursive=True
                    )
                )
                            ...
                            prior_matches.append(
                                {
                                    ...
                                    "similarity_pct": round(
                                        (1 - distance / 256) * 100, 1
                                    ),
                                }
                            )
                    except Exception as e:
                        logger.debug(f"Perceptual hash comparison failed for {path}: {e}")
                        continue
        ...
        async def device_fingerprint_db_handler(input_data: dict) -> dict:
            ...
            try:
                ...
                try:
                    ...
                    make = (
                        make_b.decode(errors="replace").strip("\x00")
                        if isinstance(make_b, bytes)
                        else ""
                    )
                    model = (
                        model_b.decode(errors="replace").strip("\x00")
                        if isinstance(model_b, bytes)
                        else ""
                    )
                    software = (
                        software_b.decode(errors="replace").strip("\x00")
                        if isinstance(software_b, bytes)
                        else ""
                    )
                    exif_raw_exif.get(piexif.ExifIFD.FocalLength)
                    exif_raw_exif.get(piexif.ExifIFD.ISOSpeedRatings)
                except Exception:
                    make, model, software = "", "", ""
                ...
                if (
                    make.startswith("Apple")
                    and not model.startswith("iPhone")
                    and not model.startswith("iPad")
                ):
                    inconsistencies.append(
                        "Apple make declared but model does not match known Apple devices."
                    )

                if make.startswith("Canon") and software and "Photoshop" in software:
                    inconsistencies.append(
                        "Canon make with Photoshop software field -- possible metadata rewrite."
                    )

                if make == "" and model != "":
                    inconsistencies.append(
                        "Model declared without Make field -- unusual for genuine captures."
                    )

                # Quick PRNU variance check
                try:
                    img_gray = np.array(
                        PILImage.open(artifact.file_path).convert("L"), dtype=np.float32
                    )
                    ...
                    if prnu_var < 0.8:
                        inconsistencies.append(
                            f"Extremely low PRNU variance ({prnu_var:.3f}) -- consistent with synthetic/GAN generation."
                        )
                except Exception:
                    prnu_var = None
                ...
                return {
                    ...
                    "prnu_variance": round(prnu_var, 4)
                    if prnu_var is not None
                    else None,
                ...
                with tempfile.TemporaryDirectory() as tmpdir:
                    ...
                    try:
                        ...
                        if gps.get(piexif.GPSIFD.GPSLatitude):
                            ...
                            exif_raw["GPS"][piexif.GPSIFD.GPSLatitude] = (
                                deg,
                                mins,
                                new_secs,
                            )
                            ...
                            gps_score = float(
                                gps_result.get("anomaly_score", orig_score)
                            )
                            perturbation_deltas["gps_jitter_0.001deg"] = round(
                                abs(gps_score - orig_score), 4
                            )
                        else:
                            perturbation_deltas["gps_jitter_0.001deg"] = 0.0

                        # 2 -- Timestamp shift +1 second
                        exif_dt = exif_raw.get("Exif", {}).get(
                        ...
                        if exif_dt:
                            ...
                            dt_str = (
                                exif_dt.decode(errors="replace")
                                if isinstance(exif_dt, bytes)
                                else exif_dt
                            )
                            try:
                                ...
                                exif_raw["Exif"][piexif.ExifIFD.DateTimeOriginal] = (
                                    new_dt
                                )
                                piexif.insert(piexif.dump(exif_raw), tmp_path)
                                ...
                                ts_score = float(
                                    ts_result.get("anomaly_score", orig_score)
                                )
                                perturbation_deltas["timestamp_+1s"] = round(
                                    abs(ts_score - orig_score), 4
                                )
                            except Exception:
        ...
        registry.register(
            ...
            extract_evidence_text_handler,
            "Auto-dispatching text extraction: PDF (PyMuPDF lossless) -> EasyOCR -> Tesseract fallback",
        )
        registry.register(
            "mediainfo_profile",
            mediainfo_profile_handler,
            "Deep AV container profiling: codec, frame rate mode, encoding tools, forensic flags",
        )
        registry.register(
            "av_file_identity",
            av_file_identity_handler,
            "Lightweight AV pre-screen: format, codec, duration, high-severity flags only",
        )
        ...
        async def gemini_deep_forensic_handler(input_data: dict) -> dict:
            ...
            try:
                ...
                if artifact.file_path.lower().endswith(
                    (".jpg", ".jpeg", ".tiff", ".tif")
                ):
                    ...
                    def _b(v):
                        return (
                            v.decode(errors="replace").strip("\x00")
                    ...
                    width = zeroth.get(piexif.ImageIFD.ImageWidth) or exif_ifd.get(
                        piexif.ExifIFD.PixelXDimension
                    )
                    height = zeroth.get(piexif.ImageIFD.ImageLength) or exif_ifd.get(
                    ...
                    if has_gps:
                        try:
                            ...
                            def _dms(val):
                                ...
                                d, m, s = val
                                return (
                                    d[0] / d[1] + m[0] / m[1] / 60 + s[0] / s[1] / 3600
                                )
                ...
                logger.debug(f"EXIF summary extraction for Gemini failed: {e}")
                exif_summary = {
                    "note": "EXIF extraction unavailable -- visual analysis only"
                }
            ...
            try:
                ...
                exif_summary.update(
                    {k: v for k, v in fs_meta.items() if v not in ("", None)}
                )
            except Exception as e:
                logger.debug(f"File system metadata retrieval failed: {e}")
            ...
            try:
                ...
                logger.debug(f"EXIF summary update failed: {e}")
            ...
            try:
                ...
                if a1:
                    ...
                    agent1_cross = {
                        k: v
                        for k, v in agent1_cross.items()
                        if v not in ("", None, [], {})
                    }
                    if agent1_cross:
                        exif_summary["agent1_image_forensics"] = agent1_cross
            except Exception as e:
                logger.debug(f"Agent1 cross-context update failed: {e}")
            ...
            result["gemini_metadata_consistency"] = getattr(
                finding, "_metadata_visual_consistency", ""
            )
        ...
        try:
            if self._tool_registry:
                ...
                if handler:
                    ...
                    device = result.get("device_model") or result.get(
                        "camera_make", ""
                    ) + " " + result.get("camera_model", "")
                    device = device.strip()
                    # software lives inside present_fields or top-level
                    present = result.get(
                        "present_fields", result.get("all_metadata", {})
                    )
                    software = result.get("software") or present.get(
                        "Software", present.get("EXIF:Software", "")
                    )
                    gps = result.get("gps_coordinates") or present.get("GPSInfo")
                    total_tags = result.get(
                        "total_fields_extracted",
                        result.get("total_exif_tags", len(present)),
                    )
                    absent_fields = result.get(
                        "absent_mandatory_fields", result.get("absent_fields", [])
                    )
                    created = (
                    ...
                    if absent_fields:
                        ...
                        context_lines.append(
                            "WARNING: No EXIF metadata found -- possible metadata stripping"
                        )
        except Exception as e:
            logger.debug(f"Build initial thought failed: {e}")

        context = (
            " | ".join(context_lines)
            if context_lines
            else "EXIF pre-screen unavailable."
        )
