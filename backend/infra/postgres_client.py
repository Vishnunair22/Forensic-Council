...
class PostgresClient:
    ...
    async def acquire_connection(self, timeout: float = 10.0, priority: int = 0):
        """
        Acquire a connection from the pool with timeout and priority.
        ...
        Raises:
            DatabaseConnectionError: If pool is exhausted
        """
        try:
            # asyncpg's acquire() already has a timeout, but let's be explicit
            ...
            logger.error(
                "Pool exhaustion: could not acquire connection within timeout",
                timeout=timeout,
                priority=priority,
    ...
    async def health_check(self) -> bool:
        """
        Test PostgreSQL connection and pool status.
